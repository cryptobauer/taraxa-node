  ...
  for (auto &lvl : db_->getNonfinalizedDagBlocks()) {
    for (auto &blk : lvl.second) {
      ...
      if (period != nullptr) {
        ...
        // Lambda that performs VRF/VDF verification for a given proposal period
        auto verifyWithProposePeriod = [&](PbftPeriod pp) -> bool {
        ...
        bool verified = verifyWithProposePeriod(*propose_period);

        // If VRF verification failed, the proposal_period_levels_map entry at this level may have
        ...
        if (!verified) {
          ...
          if (fallback_period.has_value() && *fallback_period != *propose_period) {
            ...
            verified = verifyWithProposePeriod(*fallback_period);
          }
        }
...
std::pair<DagManager::VerifyBlockReturnType, SharedTransactions> DagManager::verifyBlock(
  ...
  // Verify VDF solution
  // Lambda that attempts VRF/VDF verification with a given proposal period
  auto verifyVdfWithPeriod = [&](PbftPeriod pp) -> bool {
    const auto pk = key_manager_->getVrfKey(pp, blk->getSender());
    if (!pk) {
      return false;
    }
  ...
  bool vdf_verified = verifyVdfWithPeriod(*propose_period);
  ...
  if (!vdf_verified) {
    ...
    if (fallback_period.has_value() && *fallback_period != *propose_period) {
      LOG(log_nf_) << "DAG block " << block_hash << " VRF failed with propose_period " << *propose_period
                   << ", retrying with fallback period " << *fallback_period;
      if (verifyVdfWithPeriod(*fallback_period)) {
        vdf_verified = true;
